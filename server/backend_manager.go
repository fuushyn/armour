...
func (bm *BackendManager) discoverAndMergePluginServers() {
	...
	if len(configServers) > 0 {
		serverNames := make([]string, len(configServers))
		for i, srv := range configServers {
			serverNames[i] = srv.Name
		}
		bm.disableServersInClaudeConfig(serverNames)
	}
}

// disableServersInClaudeConfig adds server names to Claude Code's disabledMcpServers list
// so Claude Code won't also connect to them directly (since armour is proxying them).
func (bm *BackendManager) disableServersInClaudeConfig(serverNames []string) {
	homeDir := os.Getenv("HOME")
	if homeDir == "" {
		return
	}
	...
	data, err := os.ReadFile(configPath)
	if err != nil {
		bm.logger.Debug("failed to read Claude Code config for disabling: %v", err)
		return
	}

	var config map[string]interface{}
	if err := json.Unmarshal(data, &config); err != nil {
		bm.logger.Debug("failed to parse Claude Code config for disabling: %v", err)
		return
	}

	// Find all project configs and update disabledMcpServers
	...
	modified := false
	for projectPath, projectConfig := range projects {
		projMap, ok := projectConfig.(map[string]interface{})
		if !ok {
			continue
		}

		// Check if this project has mcpServers that match our discovered servers
		mcpServers, hasMcp := projMap["mcpServers"].(map[string]interface{})
		if !hasMcp {
			continue
		}
		...
		if existing, ok := projMap["disabledMcpServers"].([]interface{}); ok {
			disabled = existing
		}

		// Create a set of already disabled servers
		...
		// Add servers that exist in this project's mcpServers and aren't already disabled
		for _, serverName := range serverNames {
			if _, exists := mcpServers[serverName]; exists && !disabledSet[serverName] {
				disabled = append(disabled, serverName)
				disabledSet[serverName] = true
				modified = true
				bm.logger.Info("disabled %s in Claude Code config (proxied by armour) [%s]", serverName, projectPath)
			}
		}

		if len(disabled) > 0 {
			projMap["disabledMcpServers"] = disabled
		}
	}
	...
	if err != nil {
		bm.logger.Debug("failed to marshal updated Claude Code config: %v", err)
		return
	}

	if err := os.WriteFile(configPath, updatedData, 0600); err != nil {
		bm.logger.Debug("failed to write updated Claude Code config: %v", err)
	}
}
